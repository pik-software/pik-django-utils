...
from pik.bus.producer import CommandGeneratedEntityInstanceHandler
...
class TestCommandGeneratedEntityInstanceHandlerProduce:
    @override_settings(RABBITMQ_PRODUCES=TEST_RABBITMQ_PRODUCES)
    @patch.object(
        CommandGeneratedEntityInstanceHandler, 'host',
        new_callable=PropertyMock, return_value={})
    @patch.object(
        CommandGeneratedEntityInstanceHandler, 'headers',
        new_callable=PropertyMock, return_value={})
    def test_success(self, _host, _headers):
        entity = MyTestEntityFactory()
        request = MyTestRequestCommandFactory()
        handler = CommandGeneratedEntityInstanceHandler(
