...
from pik.api.pagination import StandardizedCursorPagination
...
class HistoryQueryParamsValidator:
    ...
    def _validate_cursor(self):
        ...
        except (ValidationError, IndexError, binascii.Error) as exc:
            raise NotFound(
                StandardizedCursorPagination.invalid_cursor_message
            ) from exc
...
class HistoryPermission(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if allow_history:
            opts = view.get_queryset().model._meta
