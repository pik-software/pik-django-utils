...
class TestDelete(TestCase):
    ...
    def test_related(self):
        depended = PermanentDepended.objects.create(dependence=self.permanent)
        self.permanent.delete()
        self.assertEqual(list(PermanentDepended.all_objects.select_related(
            'dependence').all()), [depended])

    def test_double_delete(self):
        counters = {
            'called_post_delete': 0,
            'called_post_save': 0,
        }

        def post_delete_receiver(*args, **kwargs):
            counters['called_post_delete'] += 1

        def post_save_receiver(*args, **kwargs):
            counters['called_post_save'] += 1
        ...
        # softdeleted modelds
        self.assertEqual(counters['called_post_delete'], 0)
        self.assertEqual(counters['called_post_save'], 1)
...
class TestIntegration(TestCase):
    ...
    def test_m2m_manager(self):
        ...
        PermanentM2MThrough.objects.create(
            m2m_from=_from, m2m_to=_to, deleted=now())
    ...
    def test_m2m_prefetch_related(self):
        ...
        PermanentM2MThrough.objects.create(
            m2m_from=_from, m2m_to=_to, deleted=now())
        self.assertSequenceEqual(M2MFrom.objects.prefetch_related(
            'm2mto_set').get(pk=_from.pk).m2mto_set.all(), [_to])
        self.assertEqual(M2MFrom.objects.prefetch_related(
            'm2mto_set').get(pk=_from.pk).m2mto_set.count(), 1)
    ...
    def test_m2m_all_objects(self):
        ...
        depended = NonRemovableDepended.objects.create(
            dependence=dependence, deleted=now())
...
class TestCustomQSMethods(TestCase):
    ...
    def test_update_restore_or_create__update(self):
        obj = MyPermanentModel.objects.create(name="old")
        self.assertEqual(MyPermanentModel.objects.update_restore_or_create(
            id=obj.id, defaults={'name': 'new'}).name, 'new')
